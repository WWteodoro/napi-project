...
async function handleNewAnimal(event) {
  ...
  try {
    const response = await fetch("http://127.0.0.1:3333/animal", {
      method: "POST",
...
async function loadAnimals(query = "") {
  try {
    ...
    if (query) {
      url = `http://127.0.0.1:3333/animal/name/${encodeURIComponent(query)}`;
    }
    ...
    animals.forEach(animal => {
      ...
      editBtn.addEventListener("click", () => {
        ...
        saveBtn.addEventListener("click", async () => {
          ...
          try {
            const res = await fetch(`http://127.0.0.1:3333/animal/${animal.id}`, {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
