...
async function loadLists(query = "") {
  try {
    let url = "http://127.0.0.1:3333/animalList";
    if (query) {
      url = `http://127.0.0.1:3333/animalList/name/${encodeURIComponent(query)}`;
    }
...
function startEditList(li, list) {
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:3333/animalList/${list.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
async function deleteList(id) {
  try {
    const res = await fetch(`http://127.0.0.1:3333/animalList/${id}`, {
      method: "DELETE"
    });
...
function addAnimal(listId) {
  ...
  async function loadAnimals(query = "") {
    try {
      let url = "http://127.0.0.1:3333/animal";
      if (query) url = `http://127.0.0.1:3333/animal/name/${encodeURIComponent(query)}`;
      ...
      animals.forEach(animal => {
        ...
        addBtn.addEventListener("click", async () => {
          try {
            const linkRes = await fetch(`http://127.0.0.1:3333/animalMember/animal/${animal.id}/list/${listId}`, {
              method: "POST"
            });
  ...
  newAnimalForm.onsubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:3333/animal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name })
      });
...
async function showMembersModal(list) {
  ...
  try {
    const res = await fetch(`http://127.0.0.1:3333/animalMember/list/${list.id}`);
    ...
    members.forEach(member => {
      ...
      removeBtn.addEventListener("click", async () => {
        try {
          const resDel = await fetch(`http://127.0.0.1:3333/animalMember/${member.id}/list/${list.id}`, {
            method: "DELETE"
          });
