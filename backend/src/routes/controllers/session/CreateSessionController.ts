...
export class CreateSessionController{
    ...
    async handle(req: Request, res: Response): Promise<Response>{
        const { name, animalListId, userId, latitude, longitude, placeId} = req.body;

        const createSessionService = new CreateSessionService(this.sessionRepo)
        const result  = await createSessionService.execute({ name, animalListId, userId,latitude, longitude, placeId})
